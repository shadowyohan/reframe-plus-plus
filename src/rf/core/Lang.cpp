...
constexpr Phrase kEnglish[] = {
    ...
    {"Момент из {} сохранен", "Moment from {} saved"},
    {"Повтор обрабатывается", "Processing the replay"},
    {"Доступна новая версия reframe++ {} — скачайте на GitHub",
     "reframe++ {} is available - get it on GitHub"},
    {"авто-откаты", "auto clips"},
    {"Авто-откаты", "Auto clips"},
    {"Настройка авто-откатов от поддерживаемых приложений", "Auto clips from supported apps"},
    {"Разрешить использовать авто-откаты", "Allow auto clips"},
    {"Обрезка кадра", "Crop the frame"},
    {"Обрезать кадр до размеров окна приложения", "Crop the clip to the app window"},
    {"Звук только из приложения", "App sound only"},
    {"Оставить в повторе только звук из приложения", "Keep only the app's sound in the clip"},
    {"{} поддерживает reframe++ — моменты могут сохраняться автоматически",
     "{} supports reframe++ - moments can be saved automatically"},
