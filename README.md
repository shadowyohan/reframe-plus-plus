...
- Прямой **NVENC** через `nvEncodeAPI` — пресет P5, spatial AQ, multipass,
  конвейеризация глубины 1.
- **Media Foundation** как запасной кодировщик — через него работают NVENC, AMD
  VCN и Intel QSV.
...
Вендорские SDK для сборки не нужны: заголовки NVENC вложены в репозиторий, AMF
собирается заглушкой, пока не указан путь к нему.
...
- **Прямой бэкенд AMF — заготовка**; на картах AMD запись идёт через Media
  Foundation.
...
src/rf/gpu/        устройство D3D11, преобразование цвета, особенности вендоров
